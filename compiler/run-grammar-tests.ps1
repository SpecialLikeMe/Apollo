...
$javacExe = Resolve-JavaTool -Name 'javac'
...
function Build-Compiler {
    ...
    try {
        & $javacExe -d ..\output\classes -cp ".;..\antlr-4.13.2-complete.jar" *.java
        if ($LASTEXITCODE -ne 0) {
    ...
    & $javacExe -d output\classes -cp ".;output\classes;antlr-4.13.2-complete.jar" CppCodeGenVisitor.java Main.java runtime.java RuntimePhaseOrderTest.java RuntimePhaseFailureWrapTest.java IncrementalCompileCacheTest.java
    if ($LASTEXITCODE -ne 0) {
        throw "Failed to compile Apollo compiler sources."
    }
}

function Invoke-JavaClass {
    param(
        [string]$ClassName,
        [string[]]$Arguments = @()
    )
    ...
    $argumentList = @('-cp', 'output\classes;antlr-4.13.2-complete.jar', $ClassName) + $Arguments
    try {
...
function Invoke-ApolloCompile {
    ...
    try {
        $process = Start-Process -FilePath $javaExe `
            -ArgumentList @('-cp', 'output\classes;antlr-4.13.2-complete.jar', 'Main', $resolvedPath) `
            -NoNewWindow -Wait -PassThru `
            -RedirectStandardOutput $stdoutPath `
            -RedirectStandardError $stderrPath
...
$phaseOrderResult = Invoke-JavaClass -ClassName 'RuntimePhaseOrderTest'
...
$phaseWrapResult = Invoke-JavaClass -ClassName 'RuntimePhaseFailureWrapTest'
if ($phaseWrapResult.ExitCode -ne 0) {
    throw "Runtime phase failure wrapping validation failed.`n$($phaseWrapResult.Output)"
}
Write-Host 'PASS runtime-phase-failure-wrap :: unexpected runtime phase errors become Apollo diagnostics'

$incrementalCacheResult = Invoke-JavaClass -ClassName 'IncrementalCompileCacheTest'
...
foreach ($test in $tests) {
    ...
    if ($ok -and $test.PSObject.Properties.Name -contains 'OutputMustContain') {
        $generatedOutput = if (Test-Path 'output.cpp') { [string](Get-Content -Path 'output.cpp' -Raw) } else { '' }
    ...
    if ($ok -and $test.PSObject.Properties.Name -contains 'OutputMustContainAll') {
        $generatedOutput = if (Test-Path 'output.cpp') { [string](Get-Content -Path 'output.cpp' -Raw) } else { '' }
