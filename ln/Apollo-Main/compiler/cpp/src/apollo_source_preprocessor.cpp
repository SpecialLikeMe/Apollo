    ...
    while (cursor < text.size()) {
        ...
        if (blockComment) {
            ...
            if (text[cursor] == '*' && cursor + 1 < text.size() && text[cursor + 1] == '/') {
                output.push_back(text[cursor + 1]);
                cursor += 2;
                blockComment = false;
                continue;
            }
...
std::string rewriteReservedLoopIdentifiers(std::string_view text) {
    std::string output;
    ...
    for (size_t index = 0; index < text.size();) {
        ...
        const bool startsLoop = text.substr(index, 4) == "loop";
        const bool boundaryBefore = index == 0 || !isIdentifierChar(text[index - 1]);
        const bool boundaryAfter = index + 4 >= text.size() || !isIdentifierChar(text[index + 4]);
        if (startsLoop && boundaryBefore && boundaryAfter) {
            size_t lookahead = index + 4;
            ...
            if (lookahead < text.size() && text[lookahead] == '{') {
                ...
                output.append("__apo_reserved_loop_ident");
            }
            index += 4;
            continue;
        }
...
std::string preprocessApolloSource(const std::filesystem::path& sourcePath, std::string_view sourceText) {
    ...
    const std::string reservedKeywordRewritten = rewriteReservedLoopIdentifiers(procExpanded);

    std::unordered_set<std::string> namespaceNames;
    const std::string flattened = flattenNamespaceBlocks(reservedKeywordRewritten, namespaceNames);
