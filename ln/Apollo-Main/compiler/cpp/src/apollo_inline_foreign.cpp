...
std::string normalizeClikeHeaderName(std::string header) {
    std::replace(header.begin(), header.end(), '\\', '/');
    const auto slash = header.find_last_of('/');
    if (slash != std::string::npos) {
        header = header.substr(slash + 1);
    ...
    return trimCopy(header);
}

std::vector<ApolloInlineForeignSymbol> collectKnownClikeHeaderFunctionsForHeaders(const std::vector<std::string>& headers) {
    ...
    for (const auto& header : headers) {
        const auto found = kHeaderFunctions.find(header);
        if (found == kHeaderFunctions.end()) {
            continue;
        }
        for (auto symbol : found->second) {
            appendClikeFunctionSymbol(functions, std::move(symbol));
        }
    }
    return functions;
}
...
std::vector<ApolloInlineForeignSymbol> collectKnownClikeHeaderFunctions(std::string_view preamble) {
    return collectKnownClikeHeaderFunctionsForHeaders(collectClikeIncludedHeaders(preamble));
}

std::vector<ApolloInlineForeignSymbol> collectImportedClikeSymbolsImpl(compilerv1Parser::ProgramContext* tree) {
    std::vector<std::string> headers;
    if (tree == nullptr) {
        return {};
    }
    ...
    for (auto* importStmt : tree->importStmt()) {
        ...
        if (importStmt->STRING() != nullptr) {
            header = importStmt->STRING()->getText();
            if (header.size() >= 2 && header.front() == '"' && header.back() == '"') {
                header = header.substr(1, header.size() - 2);
            }
        } else if (importStmt->headerPath() != nullptr) {
            header = importStmt->headerPath()->getText();
        }

        header = normalizeClikeHeaderName(std::move(header));
        if (!header.empty() && std::find(headers.begin(), headers.end(), header) == headers.end()) {
            headers.push_back(std::move(header));
        }
    }

    return collectKnownClikeHeaderFunctionsForHeaders(headers);
}
...
std::vector<ApolloInlineForeignSymbol> collectImportedClikeSymbols(compilerv1Parser::ProgramContext* tree) {
    return collectImportedClikeSymbolsImpl(tree);
}
