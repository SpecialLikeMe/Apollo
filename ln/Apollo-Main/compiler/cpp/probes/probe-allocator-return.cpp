...
struct apo_Buffer {
    friend void __apo_gc_trace_value(__apo_gc_tracer& tracer, const apo_Buffer& value);
    std::int32_t size;
};

inline void __apo_gc_trace_value(__apo_gc_tracer& tracer, const apo_Buffer& value) {
    __apo_gc_trace_value(tracer, value.size);
}

template <>
struct __apo_allocator_traits<apo_Buffer> {
    static constexpr bool is_memstruct = true;
    static const char* type_name() { return "Buffer"; }
};

apo_Buffer makeBuffer() {
{
        std::string arena = "arena_pool";
        const apo_Buffer value = __apo_memstruct_runtime::instance().materialize<apo_Buffer>(apo_Buffer{}, arena, "Buffer", "arena");
        return value;
    }
}
