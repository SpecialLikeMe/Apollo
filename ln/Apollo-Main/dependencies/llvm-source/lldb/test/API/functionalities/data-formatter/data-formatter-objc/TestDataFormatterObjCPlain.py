...
class ObjCDataFormatterNSPlain(ObjCDataFormatterTestCase):
    def test_plain_objc_with_run_command(self):
        ...
        self.expect("frame variable myclass", substrs=["(Class) myclass = NSValue"])
        self.expect(
            "frame variable myclass2", substrs=["(Class) myclass2 = ", "NS", "String"]
        )
        self.expect("frame variable myclass3", substrs=["(Class) myclass3 = Molecule"])
        self.expect(
            "frame variable myclass4", substrs=["(Class) myclass4 = NSMutableArray"]
        )
        self.expect("frame variable myclass5", substrs=["(Class) myclass5 = nil"])
