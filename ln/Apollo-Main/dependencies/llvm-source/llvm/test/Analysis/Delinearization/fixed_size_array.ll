...
; RUN: opt < %s -passes='print<delinearization>' -disable-output 2>&1 | FileCheck --check-prefix=CHECK %s
; RUN: opt < %s -passes='print<delinearization>' -disable-output -delinearize-use-fixed-size-array-heuristic=0 2>&1 | FileCheck --check-prefix=OFF %s
...
; OFF-LABEL: 'a_i_j2k_i'
; OFF-NEXT:  Inst: store i32 1, ptr %idx, align 4
; OFF-NEXT:  AccessFunction: {{\{\{\{}}0,+,1028}<%for.i.header>,+,256}<nw><%for.j.header>,+,128}<nw><%for.k>
; OFF-NEXT:  failed to delinearize
;
entry:
  br label %for.i.header
...
; OFF-LABEL: 'a_i_i_jk'
; OFF-NEXT:  Inst: store i32 1, ptr %idx, align 4
; OFF-NEXT:  AccessFunction: {{\{\{\{}}0,+,1152}<%for.i.header>,+,4}<nw><%for.j.header>,+,4}<nw><%for.k>
; OFF-NEXT:  failed to delinearize
;
entry:
  br label %for.i.header
