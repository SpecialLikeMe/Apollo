...
// CHECK: define{{.*}} <2 x i64> @testXMMout(ptr noundef %p) #0
...
// CHECK: define{{.*}} <4 x i64> @testYMMout(ptr noundef %p) #1
...
// CHECK: define{{.*}} <8 x i64> @testZMMout(ptr noundef %p) #2
...
// CHECK: define{{.*}} void @testXMMin(<2 x i64> noundef %xmm0, ptr noundef %p) #0
void testXMMin(__m128i xmm0, __m128i *p) {
  __asm__("vmovdqu %0, %1" : : "v"(xmm0), "m"(*(__m128i*)p));
}

// CHECK: define{{.*}} void @testYMMin(<4 x i64> noundef %ymm0, ptr noundef %p) #1
void testYMMin(__m256i ymm0, __m256i *p) {
  __asm__("vmovdqu %0, %1" : : "v"(ymm0), "m"(*(__m256i*)p));
}

// CHECK: define{{.*}} void @testZMMin(<8 x i64> noundef %zmm0, ptr noundef %p) #2
