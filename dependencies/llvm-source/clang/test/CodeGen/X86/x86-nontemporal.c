...
void test_mm_stream_ps(float* A, __m128 B) {
  ...
  // CHECK16: store <4 x float> %{{.*}}, ptr %{{.*}}, align 16, !nontemporal
  _mm_stream_ps(A, B);
}
