...
define i32 @test(ptr %P, ptr %G) nounwind {
; CHECK-LABEL: test:
; CHECK-NOT: ret
; CHECK: testl %{{.*}}, (%{{.*}})
; CHECK: ret
...
entry:
	%0 = load i32, ptr %P, align 4		; <i32> [#uses=3]
	%1 = load i32, ptr %G, align 4		; <i32> [#uses=1]
...
define i32 @test2(ptr %P, ptr %G) nounwind {
; CHECK-LABEL: test2:
; CHECK-NOT: ret
; CHECK: testl %{{.*}}, (%{{.*}})
; CHECK: ret
...
define i32 @test3(ptr %P, ptr %G) nounwind {
; CHECK-LABEL: test3:
; CHECK-NOT: ret
; CHECK: testl %{{.*}}, (%{{.*}})
; CHECK: ret
