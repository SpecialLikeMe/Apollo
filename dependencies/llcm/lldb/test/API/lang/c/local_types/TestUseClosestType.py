...
class TestUseClosestType(TestBase):
    ...
    def run_and_check_expr(self, num_children, child_type):
        ...
        result = frame.EvaluateExpression(
            "struct Foo *$mine = (struct Foo *) malloc(sizeof(struct Foo)); $mine"
        )
        self.assertSuccess(
            result.GetError(),
            "Failed to parse an expression using a multiply defined type",
        )
        self.assertEqual(
            result.GetTypeName(), "struct Foo *", "The result has the right typename."
        )
        self.assertEqual(
            result.GetNumChildren(), num_children, "Got the right number of children"
        )
        self.assertEqual(
            result.GetChildAtIndex(0).GetTypeName(), child_type, "Got the right type."
        )
    ...
    def expr_test(self):
        ...
        (target, process, self.thread, bkpt) = lldbutil.run_to_source_breakpoint(
            self, "Set a breakpoint in main", self.main_source_file
        )

        self.run_and_check_expr(3, "int")
        lldbutil.run_to_source_breakpoint(
            self, "Set a breakpoint in other", lldb.SBFileSpec("other.c")
        )
