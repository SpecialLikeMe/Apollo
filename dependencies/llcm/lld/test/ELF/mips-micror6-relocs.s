...
# RUN: llvm-objdump --no-print-imm-hex -d -t --mattr=micromips --no-show-raw-insn %teb.exe \
# RUN:   | FileCheck %s
...
# RUN: llvm-objdump --no-print-imm-hex -d -t --mattr=micromips --no-show-raw-insn %tel.exe \
# RUN:   | FileCheck %s
