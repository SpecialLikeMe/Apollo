...
; created by -passes=jump-threading (it was detected by a csmith generated program).
;
; According to -passes=verify this is valid input (even if it could be discussed if
; the dead loop really satisfies SSA form).
;
; The problem found was that the -passes=reassociate pass ends up in an infinite loop
