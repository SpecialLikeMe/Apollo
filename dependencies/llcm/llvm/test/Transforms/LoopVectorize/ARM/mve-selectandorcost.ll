...
; CHECK:       [[MIDDLE_BLOCK]]:
; CHECK-NEXT:    [[TMP16:%.*]] = call fast float @llvm.vector.reduce.fadd.v4f32(float 0.000000e+00, <4 x float> [[PREDPHI]])
...
; CHECK-NEXT:    [[TMP18:%.*]] = load float, ptr [[PB_ADDR_019]], align 4
; CHECK-NEXT:    [[CMP2:%.*]] = fcmp fast une float [[TMP17]], 0.000000e+00
; CHECK-NEXT:    [[CMP3:%.*]] = fcmp fast une float [[TMP18]], 0.000000e+00
; CHECK-NEXT:    [[OR_COND:%.*]] = select i1 [[CMP2]], i1 true, i1 [[CMP3]]
; CHECK-NEXT:    br i1 [[OR_COND]], label %[[IF_THEN:.*]], label %[[IF_END]]
; CHECK:       [[IF_THEN]]:
; CHECK-NEXT:    [[TMP19:%.*]] = tail call fast float @llvm.fabs.f32(float [[TMP17]])
; CHECK-NEXT:    [[TMP20:%.*]] = tail call fast float @llvm.fabs.f32(float [[TMP18]])
...
; CHECK:       [[WHILE_END]]:
; CHECK-NEXT:    [[ACCUM_0_LCSSA:%.*]] = phi float [ 0.000000e+00, %[[ENTRY]] ], [ [[ACCUM_1]], %[[IF_END]] ], [ [[TMP16]], %[[MIDDLE_BLOCK]] ]
