...
; RUN: opt -p loop-vectorize -force-vector-width=2 -force-target-supports-masked-memory-ops=true -tail-folding-policy=must-fold-tail -S %s | FileCheck %s --check-prefixes=CHECK,CHECK-MASKED
; RUN: opt -p loop-vectorize -force-vector-width=2 -force-target-supports-masked-memory-ops=false -tail-folding-policy=must-fold-tail -S %s | FileCheck %s --check-prefixes=CHECK,CHECK-PREDICATE
...
; CHECK-PREDICATE-NEXT:    [[TMP1:%.*]] = extractelement <2 x i1> [[TMP0]], i64 0
...
; CHECK-PREDICATE-NEXT:    [[TMP6:%.*]] = extractelement <2 x i1> [[TMP0]], i64 1
...
; CHECK-NEXT:    [[TMP2:%.*]] = extractelement <2 x i1> [[TMP0]], i64 0
...
; CHECK-NEXT:    [[TMP9:%.*]] = extractelement <2 x i64> [[TMP1]], i64 1
