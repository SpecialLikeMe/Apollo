...
;
; RUN: opt -p loop-vectorize -force-vector-width=2 -tail-folding-policy=prefer-fold-tail -S %s | FileCheck %s
; RUN: opt -p loop-vectorize -mattr=+sve -tail-folding-policy=prefer-fold-tail -S %s | FileCheck %s --check-prefix=SVE
...
; CHECK-NEXT:    [[TMP9:%.*]] = extractelement <2 x i1> [[TMP0]], i64 1
