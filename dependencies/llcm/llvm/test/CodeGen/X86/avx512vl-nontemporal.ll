...
; CHECK-NEXT:    retq ## encoding: [0xc3]
  ...
  store <8 x float> %A2, ptr %B, align 64, !nontemporal !0
  %v1 = load i32, ptr %loadptr, align 1
  %E2 = add <4 x i64> %E, %EE
  store <4 x i64> %E2, ptr %B, align 64, !nontemporal !0
  %v2 = load i32, ptr %loadptr, align 1
  %C2 = fadd <4 x double> %C, %CC
  store <4 x double> %C2, ptr %B, align 64, !nontemporal !0
...
define i32  @f128(<4 x float> %A, <4 x float> %AA, ptr %B, <2 x double> %C, <2 x double> %CC, i32 %D, <2 x i64> %E, <2 x i64> %EE, ptr %loadptr) {
; CHECK-LABEL: f128:
; CHECK:       ## %bb.0:
; CHECK-NEXT:    movl (%rdx), %eax ## encoding: [0x8b,0x02]
...
; CHECK-NEXT:    retq ## encoding: [0xc3]
  ...
  store <4 x float> %A2, ptr %B, align 64, !nontemporal !0
  %v1 = load i32, ptr %loadptr, align 1
  %E2 = add <2 x i64> %E, %EE
  store <2 x i64> %E2, ptr %B, align 64, !nontemporal !0
  %v2 = load i32, ptr %loadptr, align 1
  %C2 = fadd <2 x double> %C, %CC
  store <2 x double> %C2, ptr %B, align 64, !nontemporal !0
  %v3 = load i32, ptr %loadptr, align 1
  %sum1 = add i32 %v0, %v1
  %sum2 = add i32 %sum1, %v2
  %sum3 = add i32 %sum2, %v3
