...
define <2 x double> @f7(<2 x double> %val) {
; CHECK-LABEL: f7:
; CHECK: vfidb %v24, %v24, 4, 4
; CHECK: br %r14
  %res = call <2 x double> @llvm.roundeven.v2f64(<2 x double> %val)
  ret <2 x double> %res
}
...
define double @f13(<2 x double> %val) {
; CHECK-LABEL: f13:
...
define double @f14(<2 x double> %val) {
; CHECK-LABEL: f14:
; CHECK: wfidb %f0, %v24, 4, 4
; CHECK: br %r14
  %scalar = extractelement <2 x double> %val, i32 0
  %res = call double @llvm.roundeven.f64(double %scalar)
  ret double %res
}
