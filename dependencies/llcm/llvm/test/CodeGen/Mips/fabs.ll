...
; should be explicitly enabled with nnan flag or +abs2008 options.
...
; RUN:    -check-prefixes=CHECK,CHECK-ABSLEGACY
...
; RUN:    -check-prefixes=CHECK,CHECK-ABS2008
...
; RUN:    | FileCheck %s -check-prefixes=CHECK,CHECK-ABS2008
...
; RUN:    | FileCheck %s -check-prefixes=CHECK,CHECK-ABSLEGACY
...
; RUN:    -mattr=+abs2008,+micromips | FileCheck %s -check-prefixes=CHECK,CHECK-ABS2008
...
define float @foo0_nnan(float %a) nounwind readnone {
entry:

; CHECK-LABEL: foo0_nnan
; CHECK-ABS2008: abs.s

  %call = tail call nnan float @llvm.fabs.f32(float %a) nounwind readnone
  ret float %call
}
...
define double @foo1_nnan(double %a) nounwind readnone {
entry:

; CHECK-LABEL: foo1_nnan
; CHECK-ABS2008: abs.d

  %call = tail call nnan double @llvm.fabs.f64(double %a) nounwind readnone
  ret double %call
}
