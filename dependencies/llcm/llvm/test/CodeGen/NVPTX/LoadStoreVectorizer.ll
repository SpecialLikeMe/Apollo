...
; RUN: %if ptxas-sm_30 %{ llc < %s -mcpu=sm_30 | %ptxas-verify -arch=sm_30 %}
; RUN: %if ptxas-sm_30 %{ llc -disable-nvptx-load-store-vectorizer < %s -mcpu=sm_30 | %ptxas-verify -arch=sm_30 %}
