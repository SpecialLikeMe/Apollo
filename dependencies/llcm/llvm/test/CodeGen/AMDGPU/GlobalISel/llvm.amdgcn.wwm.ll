...
define amdgpu_ps float @wwm_f32(float %val) {
  ...
  ; GCN-NEXT:   early-clobber %2:vgpr_32 = STRICT_WWM [[COPY]], implicit $exec
...
define amdgpu_ps float @wwm_v2f16(float %arg) {
  ...
  ; GCN-NEXT: {{  $}}
  ; GCN-NEXT:   [[COPY:%[0-9]+]]:vgpr_32 = COPY $vgpr0
  ; GCN-NEXT:   early-clobber %3:vgpr_32 = STRICT_WWM [[COPY]], implicit $exec
...
define amdgpu_ps <2 x float> @wwm_f64(double %val) {
  ...
  ; GCN-NEXT:   early-clobber %4:vreg_64 = STRICT_WWM [[REG_SEQUENCE]], implicit $exec
...
define amdgpu_ps <3 x float> @wwm_v3f32(<3 x float> %val) {
  ...
  ; GCN-NEXT:   early-clobber %5:vreg_96 = STRICT_WWM [[REG_SEQUENCE]], implicit $exec
