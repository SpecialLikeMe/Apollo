...
struct __declspec(dllexport) SomeStruct {
  ...
  // M32: define weak_odr dso_local dllexport x86_thiscallcc noundef nonnull align {{[0-9]+}} dereferenceable({{[0-9]+}}) ptr @"??4SomeStruct@@QAEAAU0@ABU0@@Z"
  // M64: define weak_odr dso_local dllexport                noundef nonnull align {{[0-9]+}} dereferenceable({{[0-9]+}}) ptr @"??4SomeStruct@@QEAAAEAU0@AEBU0@@Z"
  _Atomic(int) mData;
};
