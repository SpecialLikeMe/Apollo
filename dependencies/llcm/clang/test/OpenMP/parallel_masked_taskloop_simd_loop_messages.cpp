...
// expected-error@+2 {{loop iteration variable in the associated loop of 'omp parallel masked taskloop simd' directive may not be firstprivate, predetermined as linear}}
#pragma omp parallel masked taskloop simd firstprivate(ii)
  for (ii = 0; ii < 10; ii++)
    c[ii] = a[ii];

#pragma omp parallel
// expected-error@+1 {{unexpected OpenMP clause 'in_reduction' in directive '#pragma omp parallel masked taskloop simd'}}
#pragma omp parallel masked taskloop simd in_reduction(+:ii)
  for (ii = 0; ii < 10; ii++)
    c[ii] = a[ii];

#pragma omp parallel
#pragma omp parallel masked taskloop simd linear(ii)
  for (ii = 0; ii < 10; ii++)
    c[ii] = a[ii];

#pragma omp parallel
  {
// expected-error@+2 {{loop iteration variable in the associated loop of 'omp parallel masked taskloop simd' directive may not be threadprivate or thread local, predetermined as linear}}
#pragma omp parallel masked taskloop simd
...
#pragma omp parallel
  {
#pragma omp parallel masked taskloop simd collapse(2)
    for (ii = 0; ii < 10; ii += 1)
    for (globalii = 0; globalii < 10; globalii += 1)
...
#pragma omp parallel
// omp4-error@+2 {{statement after '#pragma omp parallel masked taskloop simd' must be a for loop}}
#pragma omp parallel masked taskloop simd
  for (auto &item : a) {
    item = item + 1;
  }
...
void test_loop_firstprivate_lastprivate() {
  ...
  int c;
#pragma omp parallel
#pragma omp parallel masked taskloop simd lastprivate(s) firstprivate(s)
  for (int i = 0; i < 16; ++i)
    ;
#pragma omp parallel masked taskloop simd if(c) default(none) // expected-error {{variable 'c' must have explicitly specified data sharing attributes}} expected-note {{explicit data sharing attribute requested here}}
  for (int i = 0; i < 16; ++i)
    ;
#pragma omp parallel masked taskloop simd if(taskloop:c) default(none) // expected-error {{variable 'c' must have explicitly specified data sharing attributes}} expected-note {{explicit data sharing attribute requested here}}
  for (int i = 0; i < 16; ++i)
    ;
#pragma omp parallel masked taskloop simd if(parallel:c) default(none)
  for (int i = 0; i < 16; ++i)
    ;
}
