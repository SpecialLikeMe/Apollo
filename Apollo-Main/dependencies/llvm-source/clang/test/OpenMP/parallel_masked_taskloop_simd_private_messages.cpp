...
public:
  S4(int v) : a(v) {
#pragma omp parallel masked taskloop simd private(a) private(this->a)
    for (int k = 0; k < v; ++k)
      ++this->a;
  }
};
...
public:
  S5(int v) : a(v) {}
  S5 &operator=(S5 &s) {
#pragma omp parallel masked taskloop simd private(a) private(this->a) private(s.a) // expected-error {{expected variable name or data member of current class}}
    for (int k = 0; k < s.a; ++k)
      ++s.a;
    return *this;
  }
...
public:
  ...
  S6() : a(0) {}
  S6(T v) : a(v) {
#pragma omp parallel masked taskloop simd private(a) private(this->a) allocate(omp_thread_mem_alloc: a) // expected-warning {{allocator with the 'thread' trait access has unspecified behavior on 'parallel masked taskloop simd' directive}}
    for (int k = 0; k < v; ++k)
      ++this->a;
  }
  S6 &operator=(S6 &s) {
...
public:
  S7(T v) : a(v) {
#pragma omp parallel masked taskloop simd private(a) private(this->a) private(T::a)
    for (int k = 0; k < a.a; ++k)
      ++this->a.a;
  }
  S7 &operator=(S7 &s) {
#pragma omp parallel masked taskloop simd private(a) private(this->a) private(s.a) private(s.T::a) // expected-error 2 {{expected variable name or data member of current class}}
