...
class SmartArrayDataFormatterTestCase(TestBase):
    ...
    def data_formatter_commands(self):
        ...
        self.expect("frame variable data", substrs=["SomeData @ 0x"])
        # ${var%s}
        ...
        self.expect(
            "frame variable other.strptr", substrs=['ptr = "', "Nested Hello world!"]
        )
        ...
        self.expect(
            "frame variable other.strarr", substrs=['arr = "', "Nested Hello world!"]
        )

        self.expect("expression strarr", substrs=['arr = "', "Hello world!"])

        self.expect(
            "expression other.strarr", substrs=['arr = "', "Nested Hello world!"]
        )
        ...
        self.expect(
            "expression other.strptr", substrs=['ptr = "', "Nested Hello world!"]
        )
        ...
        self.expect(
            "frame variable strptr", matching=False, substrs=['ptr = "', "Hello world!"]
        )

        self.expect(
            "frame variable other.strptr",
            matching=False,
            substrs=['ptr = "', "Nested Hello world!"],
        )

        self.expect(
            "expression strptr", matching=False, substrs=['ptr = "', "Hello world!"]
        )

        self.expect(
            "expression other.strptr",
            matching=False,
            substrs=['ptr = "', "Nested Hello world!"],
        )
        ...
        self.expect("frame variable strptr", substrs=["ptr = ", "[{H},{e}]"])

        self.expect("frame variable other.strptr", substrs=["ptr = ", "[{N},{e}]"])

        self.expect("expression strptr", substrs=["ptr = ", "[{H},{e}]"])

        self.expect("expression other.strptr", substrs=["ptr = ", "[{N},{e}]"])
        ...
        self.expect(
            "frame variable flarr",
            substrs=["flarr = arr =", "78.5,77.25,78,76.125,76.75,76.875,77"],
        )

        self.expect(
            "frame variable other.flarr",
            substrs=["flarr = arr =", "25.5,25.25,25.125,26.75,27.375,27.5,26.125"],
        )
        ...
        self.expect(
            "frame variable flptr",
            matching=False,
            substrs=["78.5,77.25,78,76.125,76.75,76.875,77"],
        )

        self.expect("frame variable intptr", matching=False, substrs=["1,1,2,3,5"])
        ...
        if process.GetByteOrder() == lldb.eByteOrderLittle:
            self.expect(
                "frame variable intarr",
                substrs=["intarr = arr =", "....,01 00 00 00", "....,05 00 00 00"],
            )
        else:
            self.expect(
                "frame variable intarr",
                substrs=["intarr = arr =", "....,00 00 00 01", "....,00 00 00 05"],
            )

        if process.GetByteOrder() == lldb.eByteOrderLittle:
            self.expect(
                "frame variable other.intarr",
                substrs=["intarr = arr = ", "09 00 00 00", "....,07 00 00 00"],
            )
        else:
            self.expect(
                "frame variable other.intarr",
                substrs=["intarr = arr = ", "00 00 00 09", "....,00 00 00 07"],
            )
