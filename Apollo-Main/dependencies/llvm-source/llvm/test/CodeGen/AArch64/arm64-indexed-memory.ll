...
; RUN: llc < %s -mtriple=arm64-apple-ios -aarch64-enable-sink-fold=true -aarch64-redzone | FileCheck %s --check-prefixes=CHECK,CHECK-SD
; RUN: llc < %s -mtriple=arm64-apple-ios -aarch64-enable-sink-fold=true -aarch64-redzone -global-isel | FileCheck %s --check-prefixes=CHECK,CHECK-GI
...
define ptr @preidx8zext16(ptr %src, ptr %out) {
; CHECK-LABEL: preidx8zext16:
; CHECK:       ; %bb.0:
...
; CHECK-NEXT:    ret
  ...
  %ext = zext i8 %tmp to i16
  store i16 %ext, ptr %out, align 4
  ret ptr %ptr
}
...
define ptr @preidx8sext16(ptr %src, ptr %out) {
; CHECK-LABEL: preidx8sext16:
; CHECK:       ; %bb.0:
...
define ptr @postidx16_sb(ptr %src, ptr %out) {
; CHECK-LABEL: postidx16_sb:
; CHECK:       ; %bb.0:
; CHECK-NEXT:    ldrsb w8, [x0], #1
; CHECK-NEXT:    strh w8, [x1]
; CHECK-NEXT:    ret
  ...
  %ptr = getelementptr inbounds i8, ptr %src, i64 1
  %sext = sext i8 %tmp to i16
  store i16 %sext, ptr %out, align 4
  ret ptr %ptr
}
...
define ptr @postidx16_ub(ptr %src, ptr %out) {
; CHECK-LABEL: postidx16_ub:
; CHECK:       ; %bb.0:
; CHECK-NEXT:    ldrb w8, [x0], #1
; CHECK-NEXT:    strh w8, [x1]
; CHECK-NEXT:    ret
  %tmp = load i8, ptr %src, align 1
  %ptr = getelementptr inbounds i8, ptr %src, i64 1
  %zext = zext i8 %tmp to i16
  store i16 %zext, ptr %out, align 4
  ret ptr %ptr
}

define ptr @postidx32_ub(ptr %src, ptr %out) {
; CHECK-LABEL: postidx32_ub:
; CHECK:       ; %bb.0:
; CHECK-NEXT:    ldrb w8, [x0], #1
; CHECK-NEXT:    str w8, [x1]
; CHECK-NEXT:    ret
  %tmp = load i8, ptr %src, align 1
  %ptr = getelementptr inbounds i8, ptr %src, i64 1
  %zext = zext i8 %tmp to i32
  store i32 %zext, ptr %out, align 4
  ret ptr %ptr
}

define ptr @postidx32_uh(ptr %src, ptr %out) {
; CHECK-LABEL: postidx32_uh:
; CHECK:       ; %bb.0:
; CHECK-NEXT:    ldrh w8, [x0], #2
; CHECK-NEXT:    str w8, [x1]
; CHECK-NEXT:    ret
  %tmp = load i16, ptr %src, align 2
  %ptr = getelementptr inbounds i16, ptr %src, i64 1
  %zext = zext i16 %tmp to i32
  store i32 %zext, ptr %out, align 4
  ret ptr %ptr
}

define ptr @postidx64_ub(ptr %src, ptr %out) {
; CHECK-LABEL: postidx64_ub:
; CHECK:       ; %bb.0:
; CHECK-NEXT:    ldrb w8, [x0], #1
; CHECK-NEXT:    str x8, [x1]
; CHECK-NEXT:    ret
  %tmp = load i8, ptr %src, align 1
  %ptr = getelementptr inbounds i8, ptr %src, i64 1
  %zext = zext i8 %tmp to i64
  store i64 %zext, ptr %out, align 8
  ret ptr %ptr
}

define ptr @postidx64_uh(ptr %src, ptr %out) {
; CHECK-LABEL: postidx64_uh:
; CHECK:       ; %bb.0:
; CHECK-NEXT:    ldrh w8, [x0], #2
; CHECK-NEXT:    str x8, [x1]
; CHECK-NEXT:    ret
  %tmp = load i16, ptr %src, align 2
  %ptr = getelementptr inbounds i16, ptr %src, i64 1
  %zext = zext i16 %tmp to i64
  store i64 %zext, ptr %out, align 8
  ret ptr %ptr
}

define ptr @postidx64_uw(ptr %src, ptr %out) {
; CHECK-LABEL: postidx64_uw:
; CHECK:       ; %bb.0:
; CHECK-NEXT:    ldr w8, [x0], #4
; CHECK-NEXT:    str x8, [x1]
; CHECK-NEXT:    ret
  %tmp = load i32, ptr %src, align 4
  %ptr = getelementptr inbounds i32, ptr %src, i64 1
  %zext = zext i32 %tmp to i64
  store i64 %zext, ptr %out, align 8
  ret ptr %ptr
}

define ptr @postidx32_half(ptr %src, ptr %out, half %a) {
; CHECK-SD-LABEL: postidx32_half:
; CHECK-SD:       ; %bb.0:
; CHECK-SD-NEXT:    ldr h1, [x0], #4
; CHECK-SD-NEXT:    ; kill: def $h0 killed $h0 def $s0
; CHECK-SD-NEXT:    fcvt s2, h1
; CHECK-SD-NEXT:    fcmp s2, #0.0
; CHECK-SD-NEXT:    fcsel s0, s1, s0, mi
; CHECK-SD-NEXT:    str h0, [x1]
; CHECK-SD-NEXT:    ret
;
; CHECK-GI-LABEL: postidx32_half:
; CHECK-GI:       ; %bb.0:
