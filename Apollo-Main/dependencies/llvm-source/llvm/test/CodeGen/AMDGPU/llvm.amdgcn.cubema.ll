...
define amdgpu_ps float @test_cubema_vvv(float %a, float %b, float %c) #1 {
; GCN-LABEL: test_cubema_vvv:
; GCN:       ; %bb.0:
; GCN-NEXT:    v_cubema_f32 v0, v0, v1, v2
; GCN-NEXT:    ; return to shader part epilog
  %result = call float @llvm.amdgcn.cubema(float %a, float %b, float %c)
  ret float %result
}

define amdgpu_ps float @test_cubema_sss(float inreg %a, float inreg %b, float inreg %c) #1 {
; GCN-LABEL: test_cubema_sss:
; GCN:       ; %bb.0:
; GCN-NEXT:    v_mov_b32_e32 v0, s1
; GCN-NEXT:    v_mov_b32_e32 v1, s2
; GCN-NEXT:    v_cubema_f32 v0, s0, v0, v1
; GCN-NEXT:    ; return to shader part epilog
  %result = call float @llvm.amdgcn.cubema(float %a, float %b, float %c)
  ret float %result
}
