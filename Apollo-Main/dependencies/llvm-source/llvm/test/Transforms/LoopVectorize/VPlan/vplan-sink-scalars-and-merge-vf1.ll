...
; CHECK-NEXT:    vp<[[VP3:%[0-9]+]]> = DERIVED-IV ir<1024> + vp<[[VP2]]> * ir<-1>
...
; CHECK-NEXT:    EMIT-SCALAR vp<%bc.resume.val>.1 = phi [ vp<[[VP3]]>, middle.block ], [ ir<1024>, ir-bb<entry> ]
; CHECK-NEXT:  Successor(s): ir-bb<for.body>
; CHECK-EMPTY:
; CHECK-NEXT:  ir-bb<for.body>:
; CHECK-NEXT:    IR   %tmp0 = phi i64 [ %tmp6, %for.inc ], [ 0, %entry ] (extra operand: vp<%bc.resume.val> from scalar.ph)
; CHECK-NEXT:    IR   %tmp1 = phi i64 [ %tmp7, %for.inc ], [ 1024, %entry ] (extra operand: vp<%bc.resume.val>.1 from scalar.ph)
; CHECK-NEXT:    IR   %tmp2 = getelementptr i8, ptr %ptr, i64 %tmp0
; CHECK-NEXT:    IR   %tmp3 = load i8, ptr %tmp2, align 1
; CHECK-NEXT:    IR   store i8 0, ptr %tmp2, align 1
; CHECK-NEXT:    IR   %tmp4 = zext i8 %tmp3 to i32
; CHECK-NEXT:    IR   %tmp5 = trunc i32 %tmp4 to i8
; CHECK-NEXT:  No successors
; CHECK-NEXT:  }
;
entry:
