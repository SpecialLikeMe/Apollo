...
// CHECK:         %[[VAL_60:.*]] = call i32 @__kmpc_cancel(ptr @1, i32 %[[VAL_59]], i32 4)
...
// CHECK:         %[[VAL_130:.*]] = call i32 @__kmpc_cancel(ptr @1, i32 %[[VAL_129]], i32 4)
...
// Test if clause
llvm.func @_QPtest3(%arg0: !llvm.ptr {fir.bindc_name = "arg", llvm.noalias, llvm.nocapture}) {
  %0 = llvm.mlir.constant(1 : i32) : i32
  %1 = llvm.mlir.constant(100 : i32) : i32
  %2 = llvm.mlir.constant(1 : i64) : i64
  %3 = llvm.alloca %2 x i32 {bindc_name = "i"} : (i64) -> !llvm.ptr
...
// CHECK:         %[[VAL_156:.*]] = getelementptr { i64, i64, i64, ptr }, ptr %[[VAL_153]], i32 0, i32 2
// CHECK:         %[[VAL_157:.*]] = load i64, ptr %[[VAL_156]], align 4
// CHECK:         call void @__kmpc_taskloop(ptr @1, i32 %[[VAL_151]], ptr %[[VAL_152]], i32 1, ptr %[[VAL_154]], ptr %[[VAL_155]], i64 %[[VAL_157]], i32 1, i32 0, i64 0, ptr @omp_taskloop_dup.2)
// CHECK:         call void @__kmpc_end_taskgroup(ptr @1, i32 %[[VAL_151]])
// CHECK:         br label %[[VAL_158:.*]]
// CHECK:       taskloop.exit:                                    ; preds = %[[VAL_146]]
// CHECK:         ret void

// CHECK-LABEL: define internal void @_QPtest3..omp_par(
// CHECK:       taskloop.alloca:
// CHECK:         %[[VAL_159:.*]] = load ptr, ptr %[[VAL_160:.*]], align 8
// CHECK:         %[[VAL_161:.*]] = getelementptr { i64, i64, i64, ptr }, ptr %[[VAL_159]], i32 0, i32 0
// CHECK:         %[[VAL_162:.*]] = load i64, ptr %[[VAL_161]], align 4
// CHECK:         %[[VAL_163:.*]] = getelementptr { i64, i64, i64, ptr }, ptr %[[VAL_159]], i32 0, i32 1
// CHECK:         %[[VAL_164:.*]] = load i64, ptr %[[VAL_163]], align 4
// CHECK:         %[[VAL_165:.*]] = getelementptr { i64, i64, i64, ptr }, ptr %[[VAL_159]], i32 0, i32 2
// CHECK:         %[[VAL_166:.*]] = load i64, ptr %[[VAL_165]], align 4
// CHECK:         %[[VAL_167:.*]] = getelementptr { i64, i64, i64, ptr }, ptr %[[VAL_159]], i32 0, i32 3
// CHECK:         %[[VAL_168:.*]] = load ptr, ptr %[[VAL_167]], align 8, !align !1
...
// CHECK:       omp_loop.header:                                  ; preds = %[[VAL_181:.*]], %[[VAL_174]]
// CHECK:         %[[VAL_182:.*]] = phi i32 [ 0, %[[VAL_174]] ], [ %[[VAL_183:.*]], %[[VAL_181]] ]
// CHECK:         br label %[[VAL_184:.*]]
// CHECK:       omp_loop.cond:                                    ; preds = %[[VAL_180]]
// CHECK:         %[[VAL_185:.*]] = icmp ult i32 %[[VAL_182]], %[[VAL_178]]
// CHECK:         br i1 %[[VAL_185]], label %[[LOOP_BODY:omp_loop.body]], label %[[LOOP_EXIT:.*]]
...
// CHECK:       omp_loop.body:                                    ; preds = %[[VAL_184]]
// CHECK:         %[[VAL_192:.*]] = mul i32 %[[VAL_182]], 1
// CHECK:         %[[VAL_193:.*]] = add i32 %[[VAL_192]], %[[VAL_179]]
// CHECK:         br label %[[VAL_194:.*]]
// CHECK:       omp.loop_nest.region:                             ; preds = %[[LOOP_BODY]]
// CHECK:         store i32 %[[VAL_193]], ptr %[[VAL_169]], align 4
// CHECK:         call void @_QPbefore(ptr %[[VAL_172]])
// IF CLAUSE:
// CHECK:         br i1 true, label %[[CANCEL_IF_TRUE:.*]], label %[[CANCEL_IF_FALSE:.*]]
// CHECK:       [[CANCEL_IF_FALSE]]:                                                ; preds = %[[VAL_194]]
// CHECK:         %[[VAL_197:.*]] = call i32 @__kmpc_global_thread_num(ptr @1)
// CHECK:         %[[VAL_198:.*]] = call i32 @__kmpc_cancellationpoint(ptr @1, i32 %[[VAL_197]], i32 4)
// CHECK:         %[[VAL_199:.*]] = icmp eq i32 %[[VAL_198]], 0
// CHECK:         br i1 %[[VAL_199]], label %[[CANCEL_IF_FALSE_CONT:.*]], label %[[CANCEL_IF_FALSE_CANCEL:.*]]
// CHECK:       [[CANCEL_IF_FALSE_CANCEL]]:                       ; preds = %[[CANCEL_IF_FALSE]]
// CHECK:         br label %[[FINI]]
// CHECK:       [[FINI]]:                                         ; preds = %[[CANCEL_IF_TRUE_CANCEL:.*]], %[[CANCEL_IF_FALSE_CANCEL]]
// CHECK:         br label %[[OMP_REGION_CONT]]
// CHECK:       [[CANCEL_IF_FALSE_CONT]]:                         ; preds = %[[CANCEL_IF_FALSE]]
...
// CHECK:         br label %[[VAL_180]]
// CHECK:       [[CANCEL_IF_TRUE]]:                               ; preds = %[[VAL_194]]
// CHECK:         %[[VAL_206:.*]] = call i32 @__kmpc_global_thread_num(ptr @1)
// CHECK:         %[[VAL_207:.*]] = call i32 @__kmpc_cancel(ptr @1, i32 %[[VAL_206]], i32 4)
// CHECK:         %[[VAL_208:.*]] = icmp eq i32 %[[VAL_207]], 0
// CHECK:         br i1 %[[VAL_208]], label %[[CANCEL_IF_TRUE_CONT]], label %[[CANCEL_IF_TRUE_CANCEL]]
// CHECK:       [[CANCEL_IF_TRUE_CANCEL]]:                        ; preds = %[[CANCEL_IF_TRUE]]
// CHECK:         br label %[[FINI]]
// CHECK:       [[CANCEL_IF_TRUE_CONT]]:                          ; preds = %[[CANCEL_IF_TRUE]]
// CHECK:         br label %[[AFTER]]
// CHECK:       taskloop.exit.exitStub:                           ; preds = %[[OMP_REGION_CONT]]
// CHECK:         ret void
