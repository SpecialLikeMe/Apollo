...
unsigned int pick(const unsigned int left, const unsigned int right, const int chooseLeft) {
    assert(chooseLeft == 0 || chooseLeft == 1);
    return (chooseLeft == 1 ? left : right);
}

int main() {
    const std::int16_t signedSmall = 3;
    const std::uint16_t small = 7;
    const std::uint32_t selected = pick(1, 2, 1);
    const std::uint64_t widened = (selected == 1 ? 9 : 11);
    check(selected);
    std::cout << ((selected == 1 ? widened : small)) << std::endl;
    return signedSmall;
}
