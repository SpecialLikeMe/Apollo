...
static inline void log_packet(const apo_Packet packet) {
...
int main() {
    ...
    apo_cool mycool{};
__apo_executePayload(std::string("apollo\n") + std::string("") + (([&]() -> std::string { std::string __apo_stdin_0; std::getline(std::cin >> std::ws, __apo_stdin_0); return __apo_stdin_0; }())));
    return 0;
}
