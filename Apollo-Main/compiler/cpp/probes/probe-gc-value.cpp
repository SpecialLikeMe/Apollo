#include <cassert>
#include <cstdint>
#include <cstdlib>
#include <exception>
#include <filesystem>
#include <fstream>
#include <functional>
#include <iostream>
#include <memory>
#include <new>
#include <sstream>
#include <stdexcept>
#include <string>
#include <thread>
#include <unordered_map>
#include <vector>

#ifndef __APO_HASH_SUPPORT
#define __APO_HASH_SUPPORT
template <typename T>
inline void __apo_hash_combine(std::size_t& seed, const T& value) {
    seed ^= value + 0x9e3779b97f4a7c15ULL + (seed << 6) + (seed >> 2);
}
template <typename T>
struct __apo_hash {
    std::size_t operator()(const T& value) const { return std::hash<T>{}(value); }
};
template <typename T>
struct __apo_hash<std::vector<T>> {
    std::size_t operator()(const std::vector<T>& values) const {
        std::size_t seed = 0;
        for (const auto& value : values) {
            __apo_hash_combine(seed, __apo_hash<T>{}(value));
        }
        return seed;
    }
};
template <typename K, typename V, typename H, typename E, typename A>
struct __apo_hash<std::unordered_map<K, V, H, E, A>> {
    std::size_t operator()(const std::unordered_map<K, V, H, E, A>& values) const {
        std::size_t seed = 0;
        for (const auto& entry : values) {
            std::size_t entrySeed = __apo_hash<K>{}(entry.first);
            __apo_hash_combine(entrySeed, __apo_hash<V>{}(entry.second));
            seed ^= entrySeed + 0x9e3779b97f4a7c15ULL;
        }
        return seed;
    }
};
#endif

#ifndef __APO_TEMPLATE_LITERAL_SUPPORT
#define __APO_TEMPLATE_LITERAL_SUPPORT
template <typename T>
static std::string __apo_to_string(const T& value) {
    std::ostringstream stream;
    stream << value;
    return stream.str();
}
static std::string __apo_to_string(const std::string& value) { return value; }
static std::string __apo_to_string(const char* value) { return value != nullptr ? std::string(value) : std::string(); }
static std::string __apo_to_string(bool value) { return value ? "true" : "false"; }
#endif

#include "runtime_support/apo_autofmt_owner_runtime.hpp"

static __apo_autofmt_owner __apo_total_gc_owner;

struct apo_Box {
    friend void __apo_gc_trace_value(__apo_gc_tracer& tracer, const apo_Box& value);
    friend struct __apo_soa_adapter<apo_Box>;
    std::int32_t value;
    int bump() {
        return value + 1;
    }

};

inline void __apo_gc_trace_value(__apo_gc_tracer& tracer, const apo_Box& value) {
    __apo_gc_trace_value(tracer, value.value);
}

template <>
struct __apo_soa_adapter<apo_Box> {
    struct storage {
        std::vector<std::int32_t> value;
    };

    static void push(storage& storage, const apo_Box& value) {
        storage.value.push_back(value.value);
    }

    static apo_Box load(const storage& storage, std::size_t index) {
        apo_Box value{};
        value.value = storage.value[index];
        return value;
    }

    static void store(storage& storage, std::size_t index, const apo_Box& value) {
        storage.value[index] = value.value;
    }

    static std::size_t size(const storage& storage) {
        return storage.value.size();
    }
};

__apo_gc_handle<apo_Box> shared = __apo_total_gc_owner.make<apo_Box>(apo_Box{});
__apo_gc_root_slot<apo_Box> __apo_autofmt_root_shared_0(__apo_total_gc_owner, &shared);
int main() {
    GC_INIT();
    __apo_gc_handle<apo_Box> local = __apo_total_gc_owner.make<apo_Box>(apo_Box{});
__apo_gc_root_slot<apo_Box> __apo_autofmt_root_local_1(__apo_total_gc_owner, &local);
    __apo_gc_handle<apo_Box> inferred = __apo_total_gc_owner.make<apo_Box>(apo_Box{});
__apo_gc_root_slot<apo_Box> __apo_autofmt_root_inferred_2(__apo_total_gc_owner, &inferred);
    __apo_gc_handle<apo_Box> direct = __apo_total_gc_owner.make<apo_Box>(apo_Box{});
__apo_gc_root_slot<apo_Box> __apo_autofmt_root_direct_3(__apo_total_gc_owner, &direct);
    local->bump();
    inferred->bump();
    direct->bump();
    shared->bump();
    return 0;
}


