...
#include "runtime_support/apo_gui_runtime.hpp"
...
int main() {
    ...
    stage.show();
    stage.render("hero", "assets/hero.png", 10, 20);
__apo_gui_object_ref hero = stage.object_ref("hero");
    stage.render("enemy", "assets/enemy.png", 30, 20);
__apo_gui_object_ref enemy = stage.object_ref("enemy");
    if (__apo_gui_runtime::key_pressed("A")) {
        stage.hide();
    }
    stage.on("MOUSECLICK", [=]() mutable {
        const std::int32_t mouseX = __apo_gui_runtime::current_mouse_x();
        const std::int32_t mouseY = __apo_gui_runtime::current_mouse_y();
        const std::string touchedObject = __apo_gui_runtime::current_touched_object();
        if (hero.touches(enemy)) {
            std::cout << (touchedObject) << std::endl;
        }
