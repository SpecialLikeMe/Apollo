...
extern "C" {
        extern "C" int sys__printf(const char* format, ...) {
            std::string format_text = std::string(__apo_stdlib::view_or_empty(format));
            std::va_list args;
            va_start(args, format);
            const int written = std::vprintf(format_text.c_str(), args);
            va_end(args);
            std::fflush(stdout);
            return written;
        }
