...
extern long sys__json_parse_int(const char* value);
extern double sys__json_parse_float(const char* value);
namespace __apollo_inline_inline_983_5_19 {
        void* sys__url_parse(const char* value) { return __apo_stdlib::url_parse(__apo_stdlib::text_or_empty(value)); }
        const char* sys__url_scheme(void* value) { return __apo_stdlib::url_scheme(value); }
        const char* sys__url_username(void* value) { return __apo_stdlib::url_username(value); }
        const char* sys__url_password(void* value) { return __apo_stdlib::url_password(value); }
        const char* sys__url_host(void* value) { return __apo_stdlib::url_host(value); }
        const char* sys__url_port(void* value) { return __apo_stdlib::url_port(value); }
        const char* sys__url_path(void* value) { return __apo_stdlib::url_path(value); }
        const char* sys__url_query(void* value) { return __apo_stdlib::url_query(value); }
        const char* sys__url_fragment(void* value) { return __apo_stdlib::url_fragment(value); }
        const char* sys__url_join(const char* base, const char* relative) { return __apo_stdlib::url_join(__apo_stdlib::view_or_empty(base), __apo_stdlib::view_or_empty(relative)); }
